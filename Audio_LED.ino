...
void loop()
{
  ...
  for (int i = 0; i < 7; i++)
  {
    ...
    //The reason it isn't changing color is because it's only being input to one value of the RBG scale.
   ...
   //Treble LED
   analogWrite(ledPinR2, spectrumValue[1]);
   analogWrite(ledPinG2, spectrumValue[4]);
   analogWrite(ledPinB2, spectrumValue[6]);
}
